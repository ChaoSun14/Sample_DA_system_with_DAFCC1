      ...
      pom_frame_id      = CCPL_register_component(parent_comp_id, "pom","ocn", pom_mpi_comm,change_dir=.false., annotation= "register POM FRAME to c-coupler")
      call CCPL_change_to_ensemble_dir(pom_frame_id,ensemble_mem_id)
      ocean_log         = .false.
...
!      integer             :: parent_comp_id
!      parent_comp_id     = -1
!      pom_frame_id       = CCPL_register_component(parent_comp_id, "pom","ocn", pom_mpi_comm,change_dir=.true., annotation= "register POM FRAME to c-coupler")
!      ocean_log           = .false.
!      ocean_log           = CCPL_get_comp_log_file_name(pom_frame_id,log_file_name,annotation="get the ocean logfile name of pom")       
!      open(6,file=trim(log_file_name),status="UNKNOWN")
!      END SUBROUTINE register_pom_frame_coupling_configuration
...
#ifdef CCPL_DA
      comp_id           =   CCPL_register_component(parent_comp_id, comp_name,"ocn", pom_mpi_comm, annotation=annotation)
#else
      comp_id           =   CCPL_register_component(parent_comp_id, comp_name,"ocn", pom_mpi_comm, annotation=annotation)
#endif      
      pom_grid_id = 1
