      ...
      use control_wave, only: islon, ielon, islat, ielat, DELTTM
      use lims_wave, only: im, jm, kb, LON_DOM,LAT_DOM 
      use all_var_wave
      ...
      integer,             public :: ensemble_nums, ensemble_mem_id, ensemble_comm, ensemble_comp_id
      ...
      ensemble_comp_id = CCPL_register_component(-1, "WMP_member"//trim(adjustl(ensemble_mem_id_str)),"active_coupled_system", ensemble_comm, change_dir=.false., annotation = "register ensemble members of wrf-masnum-pom to C-Coupler")
      parent_comp_id   = ensemble_comp_id
      masnum_frame_id  = CCPL_register_component(parent_comp_id, "masnum","wave", masnum_mpi_comm,change_dir=.false., annotation= "register masnum FRAME to c-coupler")
      call CCPL_change_to_ensemble_dir(masnum_frame_id,ensemble_mem_id)
      wave_log         = .false.
...
!      integer             :: parent_comp_id
!      parent_comp_id     = -1
!      masnum_frame_id       = CCPL_register_component(parent_comp_id, "masnum","wave", masnum_mpi_comm,change_dir=.true., annotation= "register masnum FRAME to c-coupler")
!      wave_log        = .false.
!      wave_log          =   CCPL_get_comp_log_file_name(masnum_frame_id,log_file_name, annotation="get the wave logfile name of masnum")       
!      open(6,file=trim(log_file_name),status="UNKNOWN")
!      END SUBROUTINE register_masnum_frame_coupling_configuration
...
#ifdef CCPL_DA      
      comp_id           =   CCPL_register_component(parent_comp_id, comp_name,"wave", masnum_mpi_comm,change_dir=.false., annotation=annotation) 
#else
...
!                                                               LON_DOM(1), LON_DOM(2), LAT_DOM(1), LAT_DOM(2), masnum_state_variables(masnum_grid_id)%center_lons, masnum_state_variables(masnum_grid_id)%center_lats,&
...
!=======================================================================================================================================================================================================================================
      ...
      ! receive fields !
         ...
         !SSU(i,j) = masnum_state_variables(masnum_grid_id)%ssu_from_ocean(I,J)
         !SSV(i,j) = masnum_state_variables(masnum_grid_id)%ssv_from_ocean(I,J)
      end do
      end do
      ...
      IF (num_local_cells .gt. 0) THEN
         ...
         do i=islon,ielon
              ...
              masnum_state_variables(masnum_grid_id)%center_lats(n) = Y(j)
              masnum_state_variables(masnum_grid_id)%center_lons(n) = X(i)
...
!              if(j.ne.1 .and. j.ne.jm .and. i.ne.1 .and. i.ne.im) then
              masnum_state_variables(masnum_grid_id)%lons_vertexes(1,n) = (X(i-1)+X(i))/2 
              masnum_state_variables(masnum_grid_id)%lats_vertexes(1,n) = (Y(j-1)+Y(j))/2 

              masnum_state_variables(masnum_grid_id)%lons_vertexes(2,n) = (X(i-1)+X(i))/2 
              masnum_state_variables(masnum_grid_id)%lats_vertexes(2,n) = (Y(j+1)+Y(j))/2 

              masnum_state_variables(masnum_grid_id)%lons_vertexes(3,n) = (X(i+1)+X(i))/2 
              masnum_state_variables(masnum_grid_id)%lats_vertexes(3,n) = (Y(j+1)+Y(j))/2

              masnum_state_variables(masnum_grid_id)%lons_vertexes(4,n) = (X(i+1)+X(i))/2 
              masnum_state_variables(masnum_grid_id)%lats_vertexes(4,n) = (Y(j-1)+Y(j))/2

!              end if
!for four boundary lines
              if(j.eq.1) then
              masnum_state_variables(masnum_grid_id)%lons_vertexes(1,n) = X(i)
              masnum_state_variables(masnum_grid_id)%lats_vertexes(1,n) = Y(j)

              masnum_state_variables(masnum_grid_id)%lons_vertexes(4,n) = X(i)
              masnum_state_variables(masnum_grid_id)%lats_vertexes(4,n) = Y(j)
              end if

              if(j.eq.jm) then
              masnum_state_variables(masnum_grid_id)%lons_vertexes(2,n) = X(i)
              masnum_state_variables(masnum_grid_id)%lats_vertexes(2,n) = Y(j)

              masnum_state_variables(masnum_grid_id)%lons_vertexes(3,n) = X(i)
              masnum_state_variables(masnum_grid_id)%lats_vertexes(3,n) = Y(j)
              end if
