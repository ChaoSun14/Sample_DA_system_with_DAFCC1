...
EOF
    make -f /tmp/makefile.wrf >& /tmp/Macros
    wrf_file=${exedir}/WRFV4/configure.wrf
    ...
    sed -i "s#\(^CPPFLAGS_CCPL\s*=\s*\).*#\1 $var#" $wrf_file
...
dump_Macros "$Macfile"
rm -f $exedir/WRFV4/main/wrf.exe
./compile -j 1  em_real 
cp $exedir/WRFV4/main/wrf.exe $exedir/exe/wrf 
