...
if ($BYPASS_CONFIGURATION == 'FALSE') then
    create_symbol_copy $CODEROOT/atm/WRF4.0/WRF $DATA_DST_DIR/../WRFV4 > /dev/null
    unlink $DATA_DST_DIR/../WRFV4/configure > /dev/null
    unlink $DATA_DST_DIR/../WRFV4/compile > /dev/null
    cp $CODEROOT/atm/WRF4.0/WRF/configure $DATA_DST_DIR/../WRFV4/
    cp $CODEROOT/atm/WRF4.0/WRF/compile $DATA_DST_DIR/../WRFV4/
    cd $DATA_DST_DIR/../WRFV4
    echo "*****************************"
...
if ( ! -e "$DATA_DST_DIR/../WRFV4/run" ) then
    mkdir "$DATA_DST_DIR/../WRFV4/run"
    mkdir -p "$DATA_DST_DIR/../WRFV4/test/em_real"
    touch "$DATA_DST_DIR/../WRFV4/test/em_real/namelist.input"
endif
