...
! !USES:
    ...
    use gsi_ccpl_kinds, only:   ccpl_regional_time,ccpl_nlon_regional,ccpl_nlat_regional,ccpl_nsig,ccpl_nsig_soil,ccpl_pt,ccpl_aeta1,ccpl_eta1,ccpl_aeta2,ccpl_eta2,ccpl_glat,ccpl_glon,ccpl_dx_mc,ccpl_dy_mc
...
! This is global run
    ...
    if (ccpl_run .and. .not.io_read) then    ! ccpl online data tansmission
       write(6,*)' in init_reg_glob_ll, start ccpl online data tansmission'
...
! Get regional constants
       ...
       if(diagnostic_reg) &
           write(6,*)"in init_reg_glob_ll, ccpl_regional_time= ",ccpl_regional_time
       regional_time=ccpl_regional_time
       nlon_regional=ccpl_nlon_regional
       nlat_regional=ccpl_nlat_regional
       nsig=ccpl_nsig
       nsig_soil=ccpl_nsig_soil
       pt=ccpl_pt
...
if (.not.allocated(aeta1_ll).and..not.allocated(eta1_ll)) allocate(aeta1_ll(nsig),eta1_ll(nsig+1))
if (.not.allocated(aeta1).and..not.allocated(eta1)) allocate(aeta1(nsig),eta1(nsig+1))
if (.not.allocated(aeta2_ll).and..not.allocated(eta2_ll)) allocate(aeta2_ll(nsig),eta2_ll(nsig+1))
if (.not.allocated(aeta2).and..not.allocated(eta2)) allocate(aeta2(nsig),eta2(nsig+1))
if (.not.allocated(glat).and..not.allocated(glon)) allocate(glat(nlon_regional,nlat_regional),glon(nlon_regional,nlat_regional))
if (.not.allocated(dx_mc).and..not.allocated(dy_mc)) allocate(dx_mc(nlon_regional,nlat_regional),dy_mc(nlon_regional,nlat_regional))
       ...
       !close(lendian_in)
!ccpl_*(nlat,nlon) ==> *(nlon,nlat)      
       do i=1,nlat
          do k=1,nlon
             glat(k,i)=ccpl_glat(k,i)
             glon(k,i)=ccpl_glon(k,i)
             dx_mc(k,i)=ccpl_dx_mc(k,i)
             dy_mc(k,i)=ccpl_dy_mc(k,i)
          end do
        end do
       !glat=ccpl_glat
       !glon=ccpl_glon
...
!       nlat=nlat_regional
       ...
       if (.not.allocated(glata).and..not.allocated(glona)) allocate(glata(nlat,nlon),glona(nlat,nlon))
       if (.not.allocated(dx_mca).and..not.allocated(dy_mca)) allocate(dx_mca(nlat,nlon),dy_mca(nlat,nlon))
       ...
       if (.not.allocated(region_lat).and..not.allocated(region_lon)) allocate(region_lat(nlat,nlon),region_lon(nlat,nlon))
       if (.not.allocated(region_dy).and..not.allocated(region_dx)) allocate(region_dy(nlat,nlon),region_dx(nlat,nlon))
       if (.not.allocated(region_dyi).and..not.allocated(region_dxi)) allocate(region_dyi(nlat,nlon),region_dxi(nlat,nlon))
       if (.not.allocated(coeffy).and..not.allocated(coeffx)) allocate(coeffy(nlat,nlon),coeffx(nlat,nlon))

!   trasfer earth lats and lons to arrays region_lat, region_lon

if (.not.allocated(glat_an).and..not.allocated(glon_an)) allocate(glat_an(nlon,nlat),glon_an(nlon,nlat))
       do k=1,nlon
          do i=1,nlat
...
!       write(*,*) "ccpl_run, region_dx= ", region_dx
...
!       allocate(dx_mc(nlon_regional,nlat_regional),dy_mc(nlon_regional,nlat_regional))
       ...
       if(io_check) then
           allocate(ccpl_diff_1d(nsig+1),ccpl_diff_1d1(nsig+1))
           ccpl_diff_1d1 = eta1
           ccpl_diff_1d = ccpl_eta1
           call check_diff_1D_pointer("eta1",ccpl_diff_1d1,ccpl_diff_1d,io_ccpl_diff)
           deallocate(ccpl_diff_1d,ccpl_diff_1d1)
           allocate(ccpl_diff_1d(nsig),ccpl_diff_1d1(nsig))
           ccpl_diff_1d = aeta1
           ccpl_diff_1d1 = ccpl_aeta1
           call check_diff_1D_pointer("aeta1",ccpl_diff_1d,ccpl_diff_1d1,io_ccpl_diff)
           allocate(ccpl_diff_2D(nlon_regional,nlat_regional),ccpl_diff_2D1(nlon_regional,nlat_regional))
           ccpl_diff_2D=glat
           ccpl_diff_2D1=ccpl_glat
           call check_diff_2D_pointer("glat",ccpl_diff_2D,ccpl_diff_2D1,io_ccpl_diff)
           ccpl_diff_2D=glon
           ccpl_diff_2D1=ccpl_glon
           call check_diff_2D_pointer("glon",ccpl_diff_2D,ccpl_diff_2D1,io_ccpl_diff)
           ccpl_diff_2D=dx_mc
           ccpl_diff_2D1=ccpl_dx_mc
           call check_diff_2D_pointer("dx_mc",ccpl_diff_2D,ccpl_diff_2D1,io_ccpl_diff)
           ccpl_diff_2D=dy_mc
           ccpl_diff_2D1=ccpl_dy_mc
           call check_diff_2D_pointer("dy_mc",ccpl_diff_2D,ccpl_diff_2D1,io_ccpl_diff)
           deallocate(ccpl_diff_1d,ccpl_diff_1d1,ccpl_diff_2D,ccpl_diff_2D1)
       end if
       rlon_min_ll=one
       rlat_min_ll=one
!       nlon=nlon_regional
...
!      glons,glats are lons, lats of input grid points of dimension nlon,nlat
  ...
  if(.not.allocated(i0_tilde).and..not.allocated(j0_tilde)) allocate(i0_tilde(nxtilde,nytilde),j0_tilde(nxtilde,nytilde))
  if(.not.allocated(ip_tilde).and..not.allocated(jp_tilde)) allocate(ip_tilde(nxtilde,nytilde),jp_tilde(nxtilde,nytilde))
  if(.not.allocated(xtilde0).and..not.allocated(ytilde0)) allocate(xtilde0(nlon,nlat),ytilde0(nlon,nlat))
...
!     x-y coordinate of analysis grid is right handed).
  if(.not.allocated(clata).and..not.allocated(slata).and..not.allocated(clona).and..not.allocated(slona)) allocate(clata(nlon,nlat),slata(nlon,nlat),clona(nlon,nlat),slona(nlon,nlat))
  if(.not.allocated(cos_beta_ref).and..not.allocated(sin_beta_ref)) allocate(cos_beta_ref(nlon,nlat),sin_beta_ref(nlon,nlat))
  do j=1,nlat
     do i=1,nlon
