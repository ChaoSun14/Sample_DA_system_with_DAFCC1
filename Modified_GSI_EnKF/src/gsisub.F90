...
! Get date, grid, and other information from model guess files
  ...
  call gesinfo(mype)
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsisub (gesinfo: get date, grid, and other information) =',t2-t1,'on proc', mype
#endif /* !HAVE_ESMF */

! If single ob test, create prep.bufr file with single ob in it
  if (oneobtest) then
     if(mype==0)call oneobmakebufr
     call mpi_barrier(gsi_ccpl_comm,ierror)
  end if
...
! Read info files for assimilation of various obs
  ...
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsisub (Read info files for assimilation of various obs) =',t2-t1,'on proc', mype
...
! Compute random number for precipitation forward model.  
  ...
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsisub (Compute random number for precipitation forward model) =',t2-t1,'on proc', mype
...
! Complete setup and execute external and internal minimization loops
  ...
  if (lobserver) then
    ...
    if(last_pass) call observer_finalize()
     ...
     call glbsoi(mype)
     write(*,*) "###### destroy_gesfinfo #####"
     call destroy_gesfinfo()
     write(*,*) "###### destroy_cld_grids #####"
     call destroy_cld_grids
  endif
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsisub (Complete setup and execute external and internal minimization loops) =',t2-t1,'on proc', mype
