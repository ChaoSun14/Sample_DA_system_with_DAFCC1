...
! !USES:
  ...
  use mpimod, only: npe,ierror,mype
  ...
  use aeroinfo, only: diag_aero, init_aero, init_aero_vars, final_aero_vars, id_bias_viirs
...
!            add use of guess file later for regional mode.
  ...
  namelist/setup/gencode,factqmin,factqmax,clip_supersaturation, &
       ...
       id_bias_ps,id_bias_t,id_bias_spd, &
       conv_bias_ps,conv_bias_t,conv_bias_spd, &
       id_bias_pm2_5,conv_bias_pm2_5,id_bias_pm10,conv_bias_pm10, id_bias_viirs,&
...
!     laeroana_gocart   - when true, do chem analysis with wrfchem and modis/viirs
...
! If this is a wrf regional run, then run interface with wrf
  t1 = mpi_wtime()
  update_pint=.false.
  if (regional.and..not.ccpl_run.and.io_read) call regional_io%convert_regional_guess(mype,ctph0,stph0,tlm0)
  if (regional.and.use_gfs_stratosphere) call broadcast_gfs_stratosphere_vars
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in regional_io =',t2-t1,'on proc', mype

! Initialize variables, create/initialize arrays
  t1 = mpi_wtime()
  call init_reg_glob_ll(mype,lendian_in)
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in initialize constants required for rotation transform in case regional =',t2-t1,'on proc', mype
  t1 = mpi_wtime()
  call init_grid_vars(jcap,npe,cvars3d,cvars2d,nrf_var,mype)
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in set grid related variables =',t2-t1,'on proc', mype
...
! Initialize values in aeroinfo
  ...
  tt2 = mpi_wtime()
  if (mype == 0) print *,'time in gsimain_initialize =',tt2-tt1,'on proc', mype
...
! ! INTERFACE:
  ...
  subroutine gsimain_run(init_pass,last_pass)
    ...
    use constants, only: huge_single, one
    implicit none
...
! Call the main gsi driver routine
  ...
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsimain_run(gsisub) =',t2-t1,'on proc', mype
...
!_TBDone  call final_reg_glob_ll ! gridmod
  ...
  t2 = mpi_wtime()
  if (mype == 0) print *,'time in gsimain_finalize=',t2-t1,'on proc', mype
! Done with GSI.
  if (mype==0)  call w3tage('GSI_ANL')
  
  if (.not.ccpl_run .and. .not.io_check ) call mpi_finalize(ierror)
