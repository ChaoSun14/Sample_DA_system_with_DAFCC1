module gridinfo
  ...
  use mpisetup
  use netcdf_io
  use enkf_ccpl_kinds, only: enkf_ccpl_comm, ccpl_run, io_read
  implicit none
...
contains
  ...
  subroutine getgridinfo_arw()
    ...
    use enkf_ccpl_kinds, only: io_check
   ...
   ! Define variables returned by subroutine
    ...
    !----------------------------------------------------------------------
       ...
       call readnetcdfdata(filename,workgrid,varstringname,              &
        ...
        if(io_check) then 
            allocate(ccpl_diff_2d(dimensions%xdim,dimensions%ydim),ccpl_diff_2d1(dimensions%xdim,dimensions%ydim))
            ccpl_diff_2d=global_ccpl_xlon(:,:,1)
            ccpl_diff_2d1=workgrid(:,:,1)
            call check_diff_2D_pointer("XLONG",ccpl_diff_2d1,ccpl_diff_2d,io_ccpl_diff)
        end if
       ...
       call readnetcdfdata(filename,workgrid,varstringname,              &
        ...
        if(io_check) then
            ccpl_diff_2d=global_ccpl_xlat(:,:,1)
            ccpl_diff_2d1=workgrid(:,:,1)
            call check_diff_2D_pointer("XLAT",ccpl_diff_2d1,ccpl_diff_2d,io_ccpl_diff)
        end if
       ! Initialize counting variable
    ...
    !----------------------------------------------------------------------
       ...
       ptop = ptop/100.0
        ...
        if(io_check) then
            allocate(ccpl_diff_1d(1:dimensions%zdim),ccpl_diff_1d1(1:dimensions%zdim))
            ccpl_diff_1d=wrfarw_znu(1,1,1:dimensions%zdim)
            ccpl_diff_1d1=ccpl_znu(1:dimensions%zdim,1)
            call check_diff_1D_pointer("ZNU",ccpl_diff_1d,ccpl_diff_1d1,io_ccpl_diff)
            ccpl_diff_2d=wrfarw_mu(1:dimensions%xdim,1:dimensions%ydim,1)
            ccpl_diff_2d1=global_ccpl_mu(1:dimensions%xdim,1:dimensions%ydim,1)
            call check_diff_2D_pointer("MU",ccpl_diff_2d,ccpl_diff_2d1,io_ccpl_diff)
            ccpl_diff_2d=wrfarw_mub(1:dimensions%xdim,1:dimensions%ydim,1)
            ccpl_diff_2d1=global_ccpl_mub(1:dimensions%xdim,1:dimensions%ydim,1)
            call check_diff_2D_pointer("MUB",ccpl_diff_2d,ccpl_diff_2d1,io_ccpl_diff)
            write(*,*) "io_pt, ccpl_pt : ", workgrid(1,1,1), ccpl_pt
    ...
    call MPI_Bcast(logp,npts*nlevs_pres,mpi_real4,0,enkf_ccpl_comm,ierr)
    call MPI_Bcast(lonsgrd,npts,mpi_real4,0,enkf_ccpl_comm,ierr)
    call MPI_Bcast(latsgrd,npts,mpi_real4,0,enkf_ccpl_comm,ierr)
    call MPI_Bcast(ptop,1,mpi_real4,0,enkf_ccpl_comm,ierr)
  ...
  subroutine ccpl_getgridinfo_arw()
   ...
   ! Define variables returned by subroutine
    ...
    dimensions = griddimensions(ccpl_nlon,ccpl_nlat,ccpl_nsig)
    ...
    if(.not. allocated(wrfarw_mu)) allocate(wrfarw_mu(dimensions%xdim,dimensions%ydim,1))
    if(.not. allocated(wrfarw_mub)) allocate(wrfarw_mub(dimensions%xdim,dimensions%ydim,1))
    if(.not. allocated(wrfarw_znu)) allocate(wrfarw_znu(1,1,dimensions%zdim))
    ...
    !----------------------------------------------------------------------
       ...
       do j = 1, dimensions%ydim
          ...
          do i = 1, dimensions%xdim
             ...
             lonsgrd(count) = global_ccpl_xlon(i,j,1)*deg2rad
       ...
       do j = 1, dimensions%ydim
          ...
          do i = 1, dimensions%xdim
             ...
             latsgrd(count) =global_ccpl_xlat(i,j,1)*deg2rad
    ...
    !----------------------------------------------------------------------
       ...
       varstringname = 'ZNU'
       wrfarw_znu(1,1,1:dimensions%zdim) = ccpl_znu(1:dimensions%zdim,1)

       ! Ingest the model perturbation dry air mass from the external
       ! file

       varstringname = 'MU'
       wrfarw_mu(1:dimensions%xdim,1:dimensions%ydim,1) = global_ccpl_mu(1:dimensions%xdim,1:dimensions%ydim,1)

       ! Ingest the model base state dry air mass from the external
       ! file
       
       varstringname = 'MUB'
       wrfarw_mub(1:dimensions%xdim,1:dimensions%ydim,1) = global_ccpl_mub(1:dimensions%xdim,1:dimensions%ydim,1)
  ...
  end subroutine ccpl_getgridinfo_arw
