...
use enkf_ccpl_kinds, only: enkf_ccpl_comm, ccpl_anal_chunk, ccpl_qsat_chunk
use enkf_ccpl_kinds, only: ccpl_run, io_read, io_check
...
if (numproc < nanals) then
  ...
  call mpi_barrier(enkf_ccpl_comm,ierr)
...
if (.not.ccpl_run.and.io_read) then
...
if(io_check) then
    allocate(ccpl_diff_2d(numptsperproc(nproc+1),ndim))
    allocate(ccpl_diff_3d(nanals,numptsperproc(nproc+1),ndim))
    allocate(ccpl_diff_3d1(nanals,numptsperproc(nproc+1),ndim))
    ccpl_diff_3d=anal_chunk(:,1:numptsperproc(nproc+1),:,1)
    ccpl_anal_chunk(:,:,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs) = &
    ccpl_anal_chunk(:,:,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs)/ccpl_qsat_chunk(:,:,:)
    do nn=1,ndim
    do i=1,numptsperproc(nproc+1)
        ccpl_diff_2d(i,nn) = sum(ccpl_anal_chunk(:,i,nn))/float(nanals)
    do nanal=1,nanals
        ccpl_anal_chunk(nanal,i,nn) = ccpl_anal_chunk(nanal,i,nn)-ccpl_diff_2d(i,nn)
    end do
    end do
    end do
    write(*,*) "============================= U ============================="
    write(*,*) "ccpl_anal_chunk(1:3,10:15,1): ", ccpl_anal_chunk(1:3,10:15,1)
    write(*,*) "anal_chunk(1:3,10:15,1): ", anal_chunk(1:3,10:15,1,1)
    write(*,*) "============================= T ============================="
    write(*,*) "ccpl_anal_chunk(1:3,10:15,65): ", ccpl_anal_chunk(1:3,10:15,65)
    write(*,*) "anal_chunk(1:3,10:15,65): ", anal_chunk(1:3,10:15,65,1)
    !ccpl_diff_3d1=ccpl_anal_chunk
    !call check_diff_3D_pointer("anal_chunk",ccpl_diff_3d,ccpl_diff_3d1,io_ccpl_diff)
end if
if (nproc == 0) then
  t2 = mpi_wtime()
...
!      anal_chunk(nanal,i,:,nb) = anal_chunk(nanal,i,:,nb)+ensmean_chunk(i,:,nb)
!      anal_chunk_prior(nanal,i,:,nb) = anal_chunk_prior(nanal,i,:,nb)+ensmean_chunk_prior(i,:,nb)
!      if (pseudo_rh .and. nvarhumid > 0) then
        ! re-scale normalized spfh with sat. sphf of first guess
!        anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb) = &
!        anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb)*ccpl_qsat_chunk(nanal,i,:)
!        anal_chunk_prior(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb) = &
!        anal_chunk_prior(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb)*ccpl_qsat_chunk(nanal,i,:)
!      end if
!    enddo
!  enddo
!enddo
...
!   do i=1,numptsperproc(nproc+1)
!    do nanal=1,nanals
!      ccpl_anal_chunk(nanal,i,nn) = anal_chunk(nanal,i,nn,nb)-anal_chunk_prior(nanal,i,nn,nb)
!    enddo
!   enddo
!  enddo
!enddo
  ...
  allocate(ensmean_chunk_prior(npts_max,ndim,nbackgrounds))
  ensmean_chunk = 0.
  allocate(qsat(numptsperproc(nproc+1),nlevs,nbackgrounds))

  do nb=1,nbackgrounds ! loop over time levels in background
  do i=1,numptsperproc(nproc+1)
    anal_chunk(:,i,:,nb) = ccpl_anal_chunk(:,i,:)
    if (pseudo_rh .and. nvarhumid > 0) then
        ! create normalized humidity analysis variable.
        if (use_qsatensmean) then
          do nlev=1,nlevs
            qsat(i,nlev,nb) = sum(ccpl_qsat_chunk(:,i,nlev))/real(nanals)
          end do
          do nanal=1,nanals
            anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb) = &
            anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb)/qsat(i,:,nb)
            !anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb) = &
            !anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb)/(sum(ccpl_qsat_chunk(:,i,:),1)/real(nanals))
          end do
        else
          do nanal=1,nanals
            anal_chunk(nanal,i,(nvarhumid-1)*nlevs+1:nvarhumid*nlevs,nb) = &
  ...
  if (nproc == 0) then
    t2 = mpi_wtime()
    print *,'time to remove mean from ensemble on root',t2-t1,'secs'
  endif
  enddo ! loop over nbackgrounds
...
do nb=1,nbackgrounds ! loop over time levels in background
  ...
  call mpi_alltoallv(sendbuf, scounts, displs, mpi_real4, recvbuf, rcounts, displs,&
                     mpi_real4, enkf_ccpl_comm, ierr)
  if (nproc <= nanals-1) then
     if(.not. allocated(grdin)) allocate(grdin(npts,ndim,nbackgrounds))
     do np=1,numproc
      do nn=1,ndim
       do i=1,numptsperproc(np)
         n = ((np-1)*ndim + (nn-1))*npts_max + i
...
do nb=1,nbackgrounds
   ...
   call mpi_gatherv(sendbuf, numptsperproc(nproc+1)*ndim, mpi_real4, recvbuf, &
         scounts, displs, mpi_real4, 0, enkf_ccpl_comm, ierr)
   if (nproc == 0) then
      n = 0
      do np=1,numproc
        do nn=1,ndim
...
do nb=1,nbackgrounds
  do i=1,numptsperproc(nproc+1)
    ...
    enddo
  enddo
  ccpl_anal_chunk(:,:,:) = anal_chunk(:,:,:,nb)-anal_chunk_prior(:,:,:,nb)
enddo
  ...
  do nn=1,ndim
   do i=1,numptsperproc(nproc+1)
    do nanal=1,nanals
      ccpl_anal_chunk(nanal,i,nn) = anal_chunk(nanal,i,nn,nb)-anal_chunk_prior(nanal,i,nn,nb)
      n = ((nanal-1)*ndim + (nn-1))*npts_max + i
      sendbuf(n) = anal_chunk(nanal,i,nn,nb)
      ! convert to increment (A-F).
      sendbuf(n) = sendbuf(n)-anal_chunk_prior(nanal,i,nn,nb)
    enddo
   enddo
  enddo
!  call mpi_alltoallv(sendbuf, scounts, displs, mpi_real4, recvbuf, rcounts, displs,&
