...
use enkf_ccpl_kinds, only: enkf_ccpl_comm, ccpl_run, io_check
implicit none
! mpi definitions.
include 'mpif.h'
...
if (nproc == 0) print *,'ccpl_run: ', ccpl_run
if (.not.ccpl_run .and. .not.io_check) then
    call mpi_init(ierr)
    enkf_ccpl_comm=mpi_comm_world
end if
! nproc is process number, numproc is total number of processes.
call mpi_comm_rank(enkf_ccpl_comm,nproc,ierr)
call mpi_comm_size(enkf_ccpl_comm,numproc,ierr)
! set in GSI mpimod
mype = nproc; npe = numproc
gsi_ccpl_comm = enkf_ccpl_comm
...
CALL MPI_Comm_split_type(enkf_ccpl_comm, MPI_COMM_TYPE_SHARED, 0, &
...
call mpi_allreduce(mpi_in_place,itasks,numproc,mpi_integer,mpi_sum,enkf_ccpl_comm,ierr)
...
!endif
deallocate(itasks)
call MPI_COMM_GROUP(enkf_ccpl_comm,old_group,ierr)
call MPI_GROUP_INCL(old_group,nuse,useprocs,new_group,ierr)
deallocate(useprocs)
call MPI_COMM_CREATE(enkf_ccpl_comm,new_group,mpi_comm_shmemroot,ierr)
...
!use mpimod, only : enkf_ccpl_comm,npe,mype
...
call MPI_GROUP_INCL(old_group,nuse,useprocs,new_group,ierr)
deallocate(useprocs)
call MPI_COMM_CREATE(enkf_ccpl_comm,new_group,mpi_comm_io,ierr)
...
if (.not.ccpl_run .and. .not.io_check) call mpi_finalize(ierr)
if (ierr /= 0) then
