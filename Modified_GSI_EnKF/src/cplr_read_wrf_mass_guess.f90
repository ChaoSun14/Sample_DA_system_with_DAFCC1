...
use gsi_ccpl_kinds, only:gsi_ccpl_comm
...
contains
  ...
  !$$$  end documentation block
    ...
    use mpimod, only: mpi_sum,mpi_integer,npe,ierror, &
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
  ...
  !          set up evenly distributed index range over all processors for all input fields
          ...
          call mpi_file_open(gsi_ccpl_comm,trim(wrfges),mpi_mode_rdonly,mpi_info_null,mfcst,ierror)
...
!                                    read qnc
  ...
  !$$$
    ...
    use mpimod, only: mpi_sum,mpi_integer,mpi_real4,npe,ierror
    use guess_grids, only: &
         fact10,soil_type,veg_frac,veg_type,sfct,sno,soil_temp,soil_moi,&
         isli,nfldsig,ifilesig,ges_tsen,sfc_rough,ntguessig
    use gridmod, only: lat2,lon2,nlat_regional,nlon_regional,&
    ...
    use mpeu_util, only: die
    use gsi_ccpl_kinds, only: ccpl_psfc,ccpl_snow,ccpl_fis,ccpl_landmask,ccpl_xice,ccpl_sst,ccpl_tsk,ccpl_tslb,ccpl_smois,ccpl_ivgtyp,ccpl_vegfrac,ccpl_isltyp,ccpl_u,ccpl_v,ccpl_q,ccpl_pot
    use gsi_ccpl_kinds, only: ccpl_run, io_read, io_check
    use check_io_ccpl_diff
    implicit none
    class(read_wrf_mass_guess_class),intent(inout) :: this
  ...
  !          lm -- number of vertical levels ( = nsig for now)
   ...
   if (ccpl_run .and. .not.io_read) then
      write(6,*)' in read_wrf_mass_guess, start ccpl online data transmission'
      num_doubtful_sfct=0
      do it=1,nfldsig 
      im=nlon_regional
      jm=nlat_regional
      lm=nsig
      if(mype==0) write(6,*)' in read_wrf_mass_guess, im,jm,lm=',im,jm,lm
      ...
      call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'q' ,ges_q_it, istatus );ier=ier+istatus
      if (ier/=0) call die(trim(myname),'cannot get pointers for met-fields, ier =',ier)
      write(6,*)' in read_wrf_mass_guess, after gsi_bunle'
      call create_cld_grids()
      write(6,*)' in read_wrf_mass_guess, 0011'
      q_integral=one
      q_integralc4h=zero
      !ges_u_it=ccpl_u
      !ges_v_it=ccpl_v
      !ges_pot=ccpl_pot
      !ges_q_it=ccpl_q
      write(6,*)' in read_wrf_mass_guess, 1111'
      write(6,*)' nsig= ', nsig
      write(6,*)' size(ges_u_it): ', size(ges_u_it,1),size(ges_u_it,2),size(ges_u_it,3)
      write(6,*)' size(ges_v_it): ', size(ges_v_it,1),size(ges_v_it,2),size(ges_v_it,3)
      ...
      do k=1,nsig
         ...
         do i=1,lon2
            do j=1,lat2
               ges_u_it(j,i,k)=ccpl_u(j,i,k)
               ges_v_it(j,i,k)=ccpl_v(j,i,k)
               ges_pot(j,i,k)=ccpl_pot(j,i,k) 
               ges_q_it(j,i,k)=ccpl_q(j,i,k)
               q_integral(j,i) = q_integral(j,i)+deltasigma*ges_q_it(j,i,k)
               q_integralc4h(j,i) = q_integralc4h(j,i)+deltasigmac4h*ges_q_it(j,i,k)
  !            Convert guess mixing ratio to specific humidity
               ges_q_it(j,i,k) = ges_q_it(j,i,k)/(one+ges_q_it(j,i,k))
      ...
      write(6,*)' in read_wrf_mass_guess, finish u,v,t,q data transmission'
      do i=1,lon2
         do j=1,lat2
            soil_moi(j,i,it)=ccpl_smois(j,i)
            soil_temp(j,i,it)=ccpl_tslb(j,i)
            ges_z_it(j,i)=ccpl_fis(j,i)/grav
            psfc_this_dry=r0_01*ccpl_psfc(j,i)
            psfc_this=(psfc_this_dry-pt_ll)*q_integral(j,i)+pt_ll+q_integralc4h(j,i)
            ges_ps_it(j,i)=one_tenth*psfc_this ! convert from mb to cb

            sno(j,i,it)=ccpl_snow(j,i)
            sfc_rough(j,i,it)=rough_default
         end do
      end do
      if(mype==0) write(6,*)' in read_wrf_mass_guess, min,max(soil_moi)=', &
               minval(soil_moi),maxval(soil_moi)
      if(mype==0) write(6,*)' in read_wrf_mass_guess, min,max(soil_temp)=', &
       ...
       write(6,*)' in read_wrf_mass_guess, finish potenital temperature to temperature convertion'
       do it=1,nfldsig
       do i=1,lon2
          do j=1,lat2
             fact10(j,i,it)=one
             veg_type(j,i,it)=ccpl_ivgtyp(j,i)
             veg_frac(j,i,it)=r0_01*ccpl_vegfrac(j,i)
             soil_type(j,i,it)=ccpl_isltyp(j,i)
             sm_this=zero
             if(ccpl_landmask(j,i) /= zero_single) sm_this=one
             xice_this=zero
             if(ccpl_xice(j,i) /= zero_single) xice_this=one
             isli_this=0
             if(xice_this==one) isli_this=2
             if(xice_this==zero.and.sm_this==one) isli_this=1
             isli(j,i,it)=isli_this
             sfct(j,i,it)=ccpl_sst(j,i)
             if(isli(j,i,it) /= 0) sfct(j,i,it)=ccpl_tsk(j,i)
             if(sfct(j,i,it) < one) then
               sfct(j,i,it)=ccpl_pot(j,i,1)-300
               num_doubtful_sfct=num_doubtful_sfct+1
               if(num_doubtful_sfct <= 100) &
       ...
       end do   ! do it=1:nfldsig  
       call mpi_reduce(num_doubtful_sfct,num_doubtful_sfct_all,1,mpi_integer,mpi_sum,&
            0,gsi_ccpl_comm,ierror)
       write(6,*)' in read_wrf_mass_guess, finish ccpl online data tansmission'  
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
  ...
  !          Distribute to local domains everytime we have npe fields
             if(mod(icount,npe) == 0.or.icount==num_all_fields) then
                call mpi_alltoallv(tempa,ijn_s,displs_s,mpi_real4, &
                     all_loc(1,1,icount_prev),irc_s_reg,ird_s_reg,mpi_real4,gsi_ccpl_comm,ierror)
  ...
  !             For now, replace missing skin temps with 1st sigma level temp
       ...
       if(io_check) then
       allocate(io_sm(lat2,lon2),io_xice(lat2,lon2),io_sst(lat2,lon2),io_tsk(lat2,lon2))
       io_sm = all_loc(:,:,i_0+i_sm)
       io_xice=all_loc(:,:,i_0+i_xice)
       io_sst=all_loc(:,:,i_0+i_sst)
       io_tsk=all_loc(:,:,i_0+i_tsk)
       allocate(ccpl_diff_3d(lat2,lon2,nsig))
       call check_diff_3D_pointer("u",ges_u_it,ccpl_u,io_ccpl_diff)
       call check_diff_3D_pointer("v",ges_v_it,ccpl_v,io_ccpl_diff)
       ccpl_diff_3d=ges_pot
       call check_diff_3D_pointer("pot",ccpl_diff_3d,ccpl_pot,io_ccpl_diff)
       ccpl_diff_3d=ccpl_q/(one+ccpl_q)
       call check_diff_3D_pointer("q",ges_q_it,ccpl_diff_3d,io_ccpl_diff)
       deallocate(ccpl_diff_3d)
       allocate(ccpl_diff_2D(lat2,lon2))
       ccpl_diff_2D=soil_moi(:,:,1)
       call check_diff_2D_pointer("smois",ccpl_diff_2D,ccpl_smois,io_ccpl_diff)
       ccpl_diff_2D=soil_temp(:,:,1)
       call check_diff_2D_pointer("tslb",ccpl_diff_2D,ccpl_tslb,io_ccpl_diff)
       ccpl_diff_2D=all_loc(:,:,i_fis)
       call check_diff_2D_pointer("z",ccpl_diff_2D,ccpl_fis,io_ccpl_diff)
       ccpl_diff_2D=all_loc(:,:,i_0+i_psfc)
       call check_diff_2D_pointer("psfc",ccpl_diff_2D,ccpl_psfc,io_ccpl_diff)
       ccpl_diff_2D=sno(:,:,1)
       call check_diff_2D_pointer("sno",ccpl_diff_2D,ccpl_snow,io_ccpl_diff)
       ccpl_diff_2D=veg_type(:,:,1)
       call check_diff_2D_pointer("ivgtyp",ccpl_diff_2D,ccpl_ivgtyp,io_ccpl_diff)
       ccpl_diff_2D=all_loc(:,:,i_0+i_vegfrac)
       call check_diff_2D_pointer("veg_frac",ccpl_diff_2D,ccpl_vegfrac,io_ccpl_diff)
       ccpl_diff_2D=soil_type(:,:,1)
       call check_diff_2D_pointer("isltyp",ccpl_diff_2D,ccpl_isltyp,io_ccpl_diff)
       call check_diff_2D_pointer("landmask",io_sm,ccpl_landmask,io_ccpl_diff)
       call check_diff_2D_pointer("xice",io_xice,ccpl_xice,io_ccpl_diff)
       call check_diff_2D_pointer("sst",io_sst,ccpl_sst,io_ccpl_diff)
       call check_diff_2D_pointer("tsk",io_tsk,ccpl_tsk,io_ccpl_diff)
       deallocate(ccpl_diff_2D)
       deallocate(io_sm,io_xice,io_sst,io_tsk)
       end if
       call mpi_reduce(num_doubtful_sfct,num_doubtful_sfct_all,1,mpi_integer,mpi_sum,&
            0,gsi_ccpl_comm,ierror)
       if(mype==0) write(6,*)' in read_wrf_mass_guess, num_doubtful_sfct_all = ',num_doubtful_sfct_all
  ...
  !$$$
    ...
    use mpimod, only: ierror,mpi_real4,npe
  ...
  ! then alltoallv and i think we are done??
    ...
    call mpi_alltoallv(tempa,sendcounts,sdispls,mpi_real4, &
         all_loc,recvcounts,rdispls,mpi_real4,gsi_ccpl_comm,ierror)
  ...
  !   gather everything to ipe
       ...
       call mpi_gatherv(sendbuf,sendcount,mpi_integer,recvbuf,recvcounts, &
            displs,mpi_integer,ipe,gsi_ccpl_comm,ierror)
       if(ipe==mype) then
          ii=0
          do n=0,npe-1
             do k=k_t_start,k_t_end
